...
export default function DateTimePickerModal({
  ...
  const panResponder = PanResponder.create({
    onStartShouldSetPanResponder: (evt) => {
      // Only respond to touches in the drag handle area (top 60px)
      return evt.nativeEvent.locationY < 60;
    },
    onMoveShouldSetPanResponder: (evt, gestureState) => {
      // Only respond if starting from drag handle area and moving down
      return (
        evt.nativeEvent.locationY < 60 &&
        gestureState.dy > 20 &&
  ...
  useEffect(() => {
    ...
    const newTodayIndex = newDates.findIndex(
      (date) => date.toDateString() === selectedDate.toDateString(),
    );
  ...
  const navigateMonth = (direction: "prev" | "next") => {
    ...
    const matchingIndex = newDates.findIndex(
            ...
            >
              <View style={styles.currentSelection}>
                ...
                >
                  ...
                  <Text style={[styles.ampm, { marginLeft: 8 }]}>
